...
// Deterministic anti-fabrication backstop, shared by lib/tailor-resume.ts and
// lib/cv-chat-reformulate.ts: regardless of how well the model follows its
// prompt, a skill/tool/technology can only survive in a rewritten profile if
// its exact (case-insensitive) text already appeared somewhere in the
// source's own skills/technologies, or every one of its significant words
// appears somewhere in the source's own free text. Extracted verbatim from
// tailor-resume.ts — same behavior, just reusable.
