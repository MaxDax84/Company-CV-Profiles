...
--
-- Written idempotently (safe to re-run in full) — this specific migration
-- was confirmed to have never actually taken effect on the live project
-- despite earlier attempts (same class of issue as 0007: a run can stop
-- partway and leave things ambiguous), so re-run this exact file even if
-- some of it might already exist.
...
create unique index if not exists paid_downloads_profile_template_idx
  on public.paid_downloads (profile_id, template);
create index if not exists paid_downloads_user_id_idx on public.paid_downloads (user_id);

alter table public.paid_downloads enable row level security;

drop policy if exists paid_downloads_select_own on public.paid_downloads;
