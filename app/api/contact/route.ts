...
export async function POST(req: NextRequest) {
  ...
  try {
    await transporter.sendMail({
      from: `"Jobly Contact" <${process.env.GMAIL_USER}>`,
      to: process.env.GMAIL_USER,
      replyTo: email,
      subject: `New enquiry from ${escapeHtml(name)} — Jobly`,
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; background: #0a0b14; color: #f0f0f5; padding: 32px; border-radius: 12px; border: 1px solid #2a2d3e;">
          <div style="margin-bottom: 24px;">
            <h2 style="margin: 0 0 4px; color: #5b9cf6; font-size: 20px;">New Jobly Enquiry</h2>
